...
\begin{frame}[fragile,label={sec:org1f00e1b}]{Structure générale du dossier}
...
\begin{frame}[fragile,label={sec:orgd542c1f}]{Le fichier \texttt{chaincode.yaml}}
...
\begin{frame}[fragile,label={sec:org077023d}]{Code du fichier \texttt{chaincode.yaml}}
 \begin{verbatim}
...
\begin{frame}[fragile,label={sec:org8b035a3}]{Création du fichier de description API}
...
\begin{frame}[fragile,label={sec:orgf44198f}]{Code du fichier \texttt{hello\_world.yaml}}
 \begin{verbatim}
swagger: "2.0"
info:
  description: |
...
\begin{frame}[label={sec:orgd91ba35}]{Création du fichier du code source (en Golang)}
...
\begin{frame}[fragile,label={sec:org704d272}]{Code du fichier \texttt{hello\_world.go} :}
 \begin{verbatim}
// DISCLAIMER:
...
\begin{frame}[fragile,label={sec:org138d15e}]{Comprendre le fichier de description API}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:orgd1cb59c}]{Comprendre la correspondance verbes HTTP et appels de chaincode}
...
\begin{frame}[label={sec:orgfe154dc}]{Tableau des correspondances HTTP/Chaincode}
\begin{center}
\begin{tabular}{llll}
...
\begin{frame}[fragile,label={sec:org1206353}]{Comprendre les chemins de chaincode}
...
\begin{frame}[fragile,label={sec:orgce4dc0f}]{Définir le chemin du chaincode de \texttt{POST}}
...
\begin{frame}[fragile,label={sec:orga72bf13}]{Code du fichier \texttt{hello\_world.yaml}}
 \begin{verbatim}
consumes:
...
\begin{frame}[fragile,label={sec:org929a156}]{Explications}
...
\end{frame}
\begin{frame}[fragile,label={sec:orgceea7c6}]{Définir le chemin \texttt{GET}}
